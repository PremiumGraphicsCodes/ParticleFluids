namespace PG {
	public class MeshToPSLabels
	{
		public const string CommandNameLabel = "MeshToPS";
		public const string PolygonMeshIdLabel = "PolygonMeshId";
		public const string DivideLengthLabel = "DivideLength";
		public const string ParticleSystemIdLabel = "ParticleSystemId";
	}

	public class MeshToVoxelLabels
	{
		public const string CommandNameLabel = "MeshToVoxelCommand";
		public const string PolygonMeshIdLabel = "PolygonMeshId";
		public const string VoxelIdLabel = "VoxelId";
		public const string DivideLengthLabel = "DivideLength";
	}

	public class SparseVolumeNodeSetLabels
	{
		public const string CommandNameLabel = "SparseVolumeNodeSet";
		public const string SparseVolumeIdLabel = "SparseVolumeId";
		public const string IndicesXLabel = "IndicesX";
		public const string IndicesYLabel = "IndicesY";
		public const string IndicesZLabel = "IndicesZ";
		public const string ValuesLabel = "Values";
	}

	public class SparseVolumeSceneCreateLabels
	{
		public const string CommandNameLabel = "SparseVolumeSceneCreate";
		public const string ResolutionXLabel = "ResolutionX";
		public const string ResolutionYLabel = "ResolutionY";
		public const string ResolutionZLabel = "ResolutionZ";
		public const string BoundingBoxLabel = "BoundingBox";
		public const string NameLabel = "Name";
		public const string LayerLabel = "Layer";
		public const string NewIdLabel = "NewId";
	}

	public class VoxelNodeSetLabels
	{
		public const string CommandNameLabel = "VoxelNodeSet";
		public const string VoxelIdLabel = "VoxelId";
		public const string IndicesXLabel = "IndicesX";
		public const string IndicesYLabel = "IndicesY";
		public const string IndicesZLabel = "IndicesZ";
		public const string ValuesLabel = "Values";
	}

	public class VoxelSceneCreateLabels
	{
		public const string CommandNameLabel = "VoxelSceneCreate";
		public const string ResolutionXLabel = "ResolutionX";
		public const string ResolutionYLabel = "ResolutionY";
		public const string ResolutionZLabel = "ResolutionZ";
		public const string BoundingBoxLabel = "BoundingBox";
		public const string NameLabel = "Name";
		public const string LayerLabel = "Layer";
		public const string NewIdLabel = "NewId";
	}

	public class VoxelToPSLabels
	{
		public const string CommandNameLabel = "VoxelToPSCommand";
		public const string VoxelIdLabel = "VoxelId";
		public const string PSIdLabel = "PSId";
	}

}
